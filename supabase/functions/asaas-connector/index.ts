...
const asaasPhone=(value:unknown)=>{const digits=String(value??"").replace(/\D/g,"");const national=digits.startsWith("55")&&digits.length>11?digits.slice(2):digits;return national.length===10||national.length===11?national:undefined;};
...
async function resolveChargeJob(service:Service,organizationId:string,transactionId:string){const now=new Date().toISOString();const{error}=await service.from("asaas_charge_jobs").update({status:"succeeded",completed_at:now,last_error_code:null,updated_at:now}).eq("organization_id",organizationId).eq("transaction_id",transactionId).in("status",["pending","processing","failed"]);if(error&&!["PGRST205","42P01"].includes(String(error.code)))throw error;}
...
export default{async fetch(request:Request):Promise<Response>{
  ...
  try{
    ...
    if(action==="create_charge"){
      ...
      await resolveChargeJob(service,body.organizationId,transaction.id);await service.from("audit_logs").insert({organization_id:body.organizationId,actor_id:identity.user.id,action:"asaas.charge.created",entity:"asaas_charge",entity_id:charge.id,metadata:{transaction_id:transaction.id,amount:remaining}});return json({charge});
    }
