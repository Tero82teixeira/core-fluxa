...
async function asaas(apiKey: string, environment: string, path: string, init: RequestInit = {}) {
  const response = await fetch(`${apiBase(environment)}${path}`, {
    ...
    headers: {
      ...
      "user-agent": "FLUXA tenant billing",
      access_token: apiKey,
      ...(init.headers ?? {}),
    },
  });
...
async function authorize(
  ...
  const { data } = await service
    ...
    .eq("is_active", true)
    .maybeSingle();
  if (!data || !roles.includes(String(data.role))) throw new Error("NOT_ALLOWED");
}
async function credential(service: Service, organizationId: string) {
  ...
  return { connection, apiKey: await decrypt(secret.api_key_ciphertext, secret.api_key_iv) };
}
const financeRoles = ["superadmin", "proprietario", "administrador", "gestor", "financeiro"];
async function resolveChargeJob(service: Service, organizationId: string, transactionId: string) {
  const now = new Date().toISOString();
  const { error } = await service
    .from("asaas_charge_jobs")
    .update({ status: "succeeded", completed_at: now, last_error_code: null, updated_at: now })
    .eq("organization_id", organizationId)
...
export default {
  async fetch(request: Request): Promise<Response> {
    ...
    if (!uuid(body.organizationId)) return json({ error: "ORGANIZATION_REQUIRED" }, 400);
    ...
    try {
      ...
      if (action === "connect") {
        ...
        if (!apiKey || !uuid(body.settlementAccountId))
          throw new Error("ASAAS_CONNECTION_FIELDS_REQUIRED");
        ...
        const { data: account } = await service
          ...
          .select("id")
          .eq("id", body.settlementAccountId)
          .eq("organization_id", body.organizationId)
          .eq("is_active", true)
          .is("archived_at", null)
          .maybeSingle();
        ...
        const { data: current } = await service
          ...
          .eq("organization_id", body.organizationId)
          .maybeSingle();
        const publicToken = current?.public_token ?? crypto.randomUUID(),
          webhookToken = `${crypto.randomUUID()}${crypto.randomUUID()}`.replaceAll("-", "");
        const { data: organization } = await service
          ...
          .single();
        const webhookBody = {
          name: "FLUXA - pagamentos",
          url: `${supabaseUrl}/functions/v1/asaas-webhook?connection=${publicToken}`,
          email: organization?.email || identity.user.email,
          enabled: true,
          interrupted: false,
          apiVersion: 3,
        ...
        if (current?.webhook_id) {
          try {
            webhook = await asaas(
              apiKey,
              environment,
              `/webhooks/${encodeURIComponent(current.webhook_id)}`,
              { method: "PUT", body: JSON.stringify(webhookBody) },
            );
          } catch {
            webhook = await asaas(apiKey, environment, "/webhooks", {
              method: "POST",
        ...
        const { error: secretError } = await service.from("asaas_connection_secrets").upsert({
          organization_id: body.organizationId,
        ...
        const { data: saved, error: saveError } = await service
          ...
          .upsert(
            {
              ...
              environment,
              status: "connected",
              account_id: text(profile.id),
              account_name:
                text(profile.name) ??
                organization?.trade_name ??
                organization?.legal_name ??
                "Conta Asaas",
              settlement_account_id: body.settlementAccountId,
              webhook_id: text(webhook.id),
              webhook_token_hash: await sha256(webhookToken),
              last_checked_at: now,
              last_error_code: null,
              created_by: current?.created_by ?? identity.user.id,
              updated_by: identity.user.id,
              updated_at: now,
            },
          ...
          .select(
            "id,organization_id,environment,status,account_name,last_checked_at,settlement_account_id",
          )
          .single();
        ...
        await service.from("audit_logs").insert({
          organization_id: body.organizationId,
          actor_id: identity.user.id,
          action: "asaas.connection.saved",
          entity: "asaas_connection",
          entity_id: current?.id ?? saved.id,
      ...
      if (action === "disconnect") {
        ...
        if (connection.webhook_id)
          await asaas(
            apiKey,
            connection.environment,
            `/webhooks/${encodeURIComponent(connection.webhook_id)}`,
            { method: "DELETE" },
          ).catch(() => ({}));
        const { error } = await service
          .from("asaas_connections")
      ...
      if (action === "create_charge") {
        await authorize(service, body.organizationId, identity.user.id, financeRoles);
        if (!uuid(body.transactionId)) throw new Error("TRANSACTION_REQUIRED");
        const { connection, apiKey } = await credential(service, body.organizationId);
        ...
        if (existing) {
          await resolveChargeJob(service, body.organizationId, body.transactionId);
          return json({ charge: existing, reused: true });
        }
        const [{ data: transaction }, { data: payments }] = await Promise.all([
          service
        ...
        const paid = (payments ?? [])
          ...
          remaining = Math.round((Number(transaction.amount) - paid) * 100) / 100;
        ...
        if (!link) {
          const found = await asaas(
            ...
            foundCustomer = Array.isArray(found.data) ? record(found.data[0]) : {};
        ...
        const foundPayments = await asaas(
          ...
          foundPayment = Array.isArray(foundPayments.data) ? record(foundPayments.data[0]) : {};
            ...
            providerStatus === "RECEIVED"
              ...
              : providerStatus === "CONFIRMED"
                ...
                : providerStatus === "OVERDUE"
                  ? "overdue"
                  : "pending";
        const { data: charge, error } = await service
          .from("asaas_charges")
          .upsert(
            {
              organization_id: body.organizationId,
              ...
              last_event_type: "PAYMENT_CREATED",
          ...
          .select("*")
          .single();
        if (error || !charge?.invoice_url) throw error ?? new Error("ASAAS_INVOICE_URL_MISSING");
        if (status === "received") {
          const { error: reconciliationError } = await service.rpc("apply_asaas_payment_event", {
            _connection_token: connection.public_token,
            _event_id: `connector-recovery-${providerPayment.id}`,
            _event_type: "PAYMENT_RECEIVED",
        ...
        await resolveChargeJob(service, body.organizationId, transaction.id);
        await service.from("audit_logs").insert({
          organization_id: body.organizationId,
          actor_id: identity.user.id,
          action: "asaas.charge.created",
          entity: "asaas_charge",
          entity_id: charge.id,
          metadata: { transaction_id: transaction.id, amount: remaining },
        });
        return json({ charge });
      }
      if (action === "sync_charge") {
        await authorize(service, body.organizationId, identity.user.id, financeRoles);
        if (!uuid(body.chargeId)) throw new Error("CHARGE_REQUIRED");
        const { connection, apiKey } = await credential(service, body.organizationId);
        const { data: charge } = await service
          ...
          .eq("organization_id", body.organizationId)
          .maybeSingle();
        if (!charge) throw new Error("ASAAS_CHARGE_NOT_FOUND");
        const providerPayment = await asaas(
            apiKey,
            connection.environment,
            `/payments/${encodeURIComponent(charge.provider_payment_id)}`,
          ),
          providerStatus = String(providerPayment.status ?? "PENDING").toUpperCase();
        const eventType =
          providerStatus === "RECEIVED"
              ...
              : providerStatus === "OVERDUE"
                ...
                : providerStatus === "REFUNDED"
                  ...
                  : providerStatus.includes("CHARGEBACK")
                    ...
                    : "PAYMENT_SYNC";
        const paidAtValue =
            text(providerPayment.paymentDate) ?? text(providerPayment.confirmedDate),
          paidAt = paidAtValue ? new Date(`${paidAtValue}T12:00:00Z`).toISOString() : null;
        const { error: syncError } = await service.rpc("apply_asaas_payment_event", {
          _connection_token: connection.public_token,
          _event_id: `manual-sync-${charge.provider_payment_id}-${providerStatus}`,
          _event_type: eventType,
          _provider_payment_id: charge.provider_payment_id,
          _provider_status: providerStatus,
          _paid_at: paidAt,
          _amount: Number(providerPayment.value ?? charge.amount),
        });
        if (syncError) throw new Error("ASAAS_RECONCILIATION_FAILED");
        await service
          .from("asaas_connections")
          .update({
        ...
        const { data: synced } = await service
          ...
          .eq("id", charge.id)
          .single();
        return json({ charge: synced });
      }
      if (action === "retry_charge_job") {
        await authorize(service, body.organizationId, identity.user.id, financeRoles);
        if (!uuid(body.jobId)) throw new Error("ASAAS_JOB_REQUIRED");
        const now = new Date().toISOString();
        const { data: job, error } = await service
          ...
          .maybeSingle();
        if (error) throw error;
        if (!job) throw new Error("ASAAS_JOB_NOT_RETRYABLE");
        await service.from("audit_logs").insert({
          organization_id: body.organizationId,
          actor_id: identity.user.id,
          action: "asaas.charge.retry_requested",
          entity: "asaas_charge_job",
          entity_id: job.id,
          metadata: {},
        });
        return json({ job });
      ...
      if (action === "cancel_charge") {
        ...
        const { data: charge } = await service
          ...
          .maybeSingle();
        if (!charge) throw new Error("ASAAS_CHARGE_NOT_FOUND");
        if (!["pending", "confirmed", "overdue"].includes(charge.status))
          throw new Error("ASAAS_CHARGE_NOT_CANCELLABLE");
        await asaas(
          apiKey,
          connection.environment,
          `/payments/${encodeURIComponent(charge.provider_payment_id)}`,
        ...
        const { error } = await service
          ...
          .update({
            status: "cancelled",
            last_event_type: "PAYMENT_DELETED_BY_USER",
            last_event_at: new Date().toISOString(),
            updated_at: new Date().toISOString(),
          })
          .eq("id", charge.id)
      ...
      const code =
        error instanceof Error
          ? error.message.split(" ")[0].slice(0, 140)
          : "ASAAS_OPERATION_FAILED";
      console.error(JSON.stringify({ source: "asaas-connector", action, code }));
      return json(
        { error: code },
        code === "NOT_ALLOWED"
          ? 403
          : code.includes("REQUIRED") || code.includes("ELIGIBLE")
            ? 422
            : 502,
      );
    }
