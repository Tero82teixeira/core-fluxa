...
SELECT ok((SELECT indexdef LIKE '%(organization_id, monitoring_status, updated_at DESC)%' FROM pg_indexes WHERE schemaname='public' AND indexname='monitoring_states_org_status_idx'), 'organization/status monitoring index keeps the expected columns');
