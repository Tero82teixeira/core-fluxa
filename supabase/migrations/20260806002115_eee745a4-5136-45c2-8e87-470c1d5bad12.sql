...
BEGIN
  ...
  IF v_inv.expires_at <= now() THEN
    UPDATE public.organization_invitations i
     ...
     WHERE i.id = v_inv.id AND i.status = 'pending';
  ...
  IF FOUND THEN
    UPDATE public.organization_members m
       SET is_active = true,
           role = CASE WHEN m.is_active THEN m.role ELSE v_inv.role END,
           updated_at = now()
  ...
  ELSE
    ...
    VALUES (v_inv.organization_id, v_uid, v_inv.role, true)
  ...
  VALUES (v_inv.organization_id, v_uid, v_name, 'invite.accepted', 'member', v_membership.user_id,
  ...
  IF v_inv.invited_by IS NOT NULL THEN
    INSERT INTO public.notifications (organization_id, user_id, kind, title, body, action_url, dedupe_key)
    VALUES (v_inv.organization_id, v_inv.invited_by, 'team', 'Convite aceito',
            v_name || ' entrou para a equipe.', '/equipe', 'invite-accepted-' || v_inv.id::text)
