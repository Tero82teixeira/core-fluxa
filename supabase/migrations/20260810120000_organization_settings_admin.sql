...
ALTER TABLE public.organization_settings
  ...
  ADD COLUMN IF NOT EXISTS default_communication_channel text,
  ADD COLUMN IF NOT EXISTS default_communication_priority public.priority_level,
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'default_communication_channel',COALESCE(s.default_communication_channel,'interno'),'default_communication_priority',COALESCE(s.default_communication_priority,'media'),'default_follow_up_hours',COALESCE(s.default_follow_up_hours,24),'highlight_internal_notes',COALESCE(s.highlight_internal_notes,true),
...
BEGIN
  ...
  IF _changes->>'default_responsible_id' IS NOT NULL AND NOT EXISTS(SELECT 1 FROM organization_members WHERE organization_id=_organization_id AND user_id=(_changes->>'default_responsible_id')::uuid AND is_active) THEN RAISE EXCEPTION 'SETTINGS_RELATED_ID_ORG_MISMATCH'; END IF;
  IF _changes->>'default_financial_account_id' IS NOT NULL AND NOT EXISTS(SELECT 1 FROM financial_accounts WHERE organization_id=_organization_id AND id=(_changes->>'default_financial_account_id')::uuid) THEN RAISE EXCEPTION 'SETTINGS_RELATED_ID_ORG_MISMATCH'; END IF;
  IF _changes->>'default_income_category_id' IS NOT NULL AND NOT EXISTS(SELECT 1 FROM financial_categories WHERE organization_id=_organization_id AND id=(_changes->>'default_income_category_id')::uuid AND type='income') THEN RAISE EXCEPTION 'SETTINGS_RELATED_ID_ORG_MISMATCH'; END IF;
  IF _changes->>'default_expense_category_id' IS NOT NULL AND NOT EXISTS(SELECT 1 FROM financial_categories WHERE organization_id=_organization_id AND id=(_changes->>'default_expense_category_id')::uuid AND type='expense') THEN RAISE EXCEPTION 'SETTINGS_RELATED_ID_ORG_MISMATCH'; END IF;
