...
-- Keep the single existing fifteen-minute database clock while appending this
-- stage without copying or silently dropping any previous automation.
ALTER FUNCTION public.run_temporal_automation_cycle()
  RENAME TO run_temporal_automation_cycle_before_asaas_collection;
...
BEGIN
  previous_result := public.run_temporal_automation_cycle_before_asaas_collection();
  BEGIN
...
REVOKE ALL ON FUNCTION public.run_temporal_automation_cycle_before_asaas_collection()
  FROM PUBLIC,anon,authenticated,service_role;
REVOKE ALL ON FUNCTION public.run_temporal_automation_cycle()
  FROM PUBLIC,anon,authenticated,service_role;
GRANT EXECUTE ON FUNCTION public.run_temporal_automation_cycle_before_asaas_collection() TO postgres;
