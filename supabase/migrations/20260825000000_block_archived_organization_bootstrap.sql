...
BEGIN
  ...
  -- Vínculos de organizações arquivadas nunca participam da seleção normal.
  ...
  IF v_membership.id IS NULL THEN
    -- Um convite válido continua tendo precedência sobre a criação de workspace.
    IF EXISTS (
      SELECT 1
      ...
      RAISE EXCEPTION 'BOOTSTRAP_INVITATION_PENDING' USING ERRCODE = 'P0001';
    END IF;

    -- Não reativa o vínculo arquivado nem cria outra empresa silenciosamente.
    IF EXISTS (
      SELECT 1
        FROM public.organization_members m
        JOIN public.organizations o ON o.id = m.organization_id
      ...
      RAISE EXCEPTION 'BOOTSTRAP_ORGANIZATION_ARCHIVED' USING ERRCODE = '28000';
    END IF;

    SELECT o.* INTO v_org
      FROM public.organizations o
     WHERE o.created_by = v_caller
       AND o.archived_at IS NULL
  ...
  ELSE
    ...
    IF NOT v_membership.is_active
       AND (v_membership.role = 'proprietario' OR v_org.created_by = v_caller) THEN
    ...
    IF v_org.created_by IS NULL AND v_membership.role = 'proprietario' THEN
      UPDATE public.organizations o
         SET created_by = v_caller
       WHERE o.id = v_org.id
       RETURNING o.* INTO v_org;
    END IF;
  END IF;
