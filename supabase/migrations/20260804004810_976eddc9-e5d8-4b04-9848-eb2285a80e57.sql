...
BEGIN
 FOR c IN SELECT value FROM jsonb_array_elements(_conditions) LOOP
  ...
  fails := CASE c->>'operator'
    ...
    ELSE true
  END;
  IF fails THEN RETURN false; END IF;
 END LOOP;
 RETURN true;
END $$;
...
BEGIN
 ...
 FOR r IN SELECT * FROM public.automation_rules WHERE organization_id=_organization_id AND trigger_type=_event_type AND is_active AND archived_at IS NULL AND (id IS DISTINCT FROM _source_automation_rule_id) ORDER BY created_at LOOP
  key:=encode(extensions.digest(r.id::text||':'||coalesce(_entity_id::text,'none')||':'||_event_type||':'||coalesce(_event_version,_payload::text),'sha256'),'hex');
  BEGIN
...
DROP TRIGGER IF EXISTS tasks_automation_events ON public.tasks;
...
DROP TRIGGER IF EXISTS monitoring_automation_events ON public.monitoring_items;
