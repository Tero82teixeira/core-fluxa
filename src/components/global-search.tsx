...
import { Compass, FileStack, Gauge, HelpCircle, ListChecks, Loader2, MessagesSquare, Search, Sparkles, Users, Wallet, Files } from "lucide-react";

import { CommandDialog, CommandEmpty, CommandGroup, CommandInput, CommandItem, CommandList } from "@/components/ui/command";
import { useGlobalSearch } from "@/hooks/use-operations";
import { composeGlobalSearchResults, type GlobalSearchResult, type GlobalSearchType } from "@/lib/global-search";
import { useWorkspace } from "@/lib/workspace";

const GROUPS: GlobalSearchType[] = ["Navegação", "Cliente", "Processo", "Tarefa", "Documento", "Comunicação", "Financeiro", "Monitoramento", "Ajuda", "Novidade"];
const LABELS: Record<GlobalSearchType, string> = { Navegação: "Navegação", Cliente: "Clientes", Processo: "Processos", Tarefa: "Tarefas", Documento: "Documentos", Comunicação: "Comunicação", Financeiro: "Financeiro", Monitoramento: "Monitoramento", Ajuda: "Ajuda", Novidade: "Novidades" };
const ICONS = { Navegação: Compass, Cliente: Users, Processo: FileStack, Tarefa: ListChecks, Documento: Files, Comunicação: MessagesSquare, Financeiro: Wallet, Monitoramento: Gauge, Ajuda: HelpCircle, Novidade: Sparkles } satisfies Record<GlobalSearchType, typeof Search>;
...
export function GlobalSearch({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const currentRemoteResults = normalizedTerm === debouncedTerm ? remote.data ?? [] : [];
  const results = useMemo(() => composeGlobalSearchResults(normalizedTerm, access, currentRemoteResults), [normalizedTerm, access, currentRemoteResults]);
  ...
  return (
    <CommandDialog open={open} onOpenChange={onOpenChange}>
      ...
      <CommandList className="max-h-[min(70dvh,32rem)]">
        ...
        {loading && <div role="status" className="flex items-center justify-center gap-2 px-4 py-3 text-xs text-muted-foreground"><Loader2 className="size-3.5 animate-spin" />Buscando na organização...</div>}
