...
import { FOCUSED_HEALTH_NOTIFICATION_KINDS, type Notification } from "@/lib/notifications";
...
export function useNotifications(organizationId: string | null, limit = 20, focusedHealth = false) {
  return useQuery({
    enabled: Boolean(organizationId),
    queryKey: [...key(organizationId), limit, focusedHealth],
    ...
    queryFn: async (): Promise<Notification[]> => {
      let query = db()
        ...
        .is("archived_at", null);
      if (focusedHealth) query = query.in("kind", FOCUSED_HEALTH_NOTIFICATION_KINDS);
...
export function useUnreadNotificationCount(organizationId: string | null, focusedHealth = false) {
  return useQuery({
    enabled: Boolean(organizationId),
    queryKey: [...key(organizationId), "unread-count", focusedHealth],
    refetchInterval: 60_000,
    queryFn: async () => {
      let query = db()
        .from("notifications")
