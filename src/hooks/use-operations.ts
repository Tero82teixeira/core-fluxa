  ...
  return useQuery({
    ...
    queryFn: async (): Promise<GlobalSearchResult[]> => {
      ...
      const sources = await Promise.allSettled([
        ...
        source("documentos", operational, () => db().from("documents").select("id,title,internal_code,document_number,original_file_name,status,clients(name),processes(code),document_types(name)").eq("organization_id", organizationId).is("archived_at", null).or(`title.ilike.${like},internal_code.ilike.${like},document_number.ilike.${like},original_file_name.ilike.${like}`).limit(5), (r) => ({ id: r.id, type: "Documento", title: r.title || r.original_file_name, subtitle: [r.internal_code, r.document_types?.name, r.clients?.name, r.processes?.code].filter(Boolean).join(" · "), keywords: [r.internal_code, r.document_number, r.original_file_name], route: "/documentos" })),
