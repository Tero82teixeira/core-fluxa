...
export function useUpdateCommunicationThread(organizationId: string | null) { const refresh = useRefresh(organizationId); return useMutation({ mutationFn: async ({ threadId, followUpAt, clearFollowUp = false }: { threadId: string; followUpAt?: string | null; clearFollowUp?: boolean }) => { const { error } = await db().rpc("update_communication_thread", { _thread_id: threadId, _follow_up_at: followUpAt || null, _clear_follow_up: clearFollowUp }); if (error) throw error; }, onSuccess: (_, v) => refresh(v.threadId) }); }
