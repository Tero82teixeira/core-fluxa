...
function useRefresh(organizationId: string | null) { const client = useQueryClient(); return (threadId?: string) => { client.invalidateQueries({ queryKey: ["communication-threads", organizationId] }); client.invalidateQueries({ queryKey: ["communication-search", organizationId] }); client.invalidateQueries({ queryKey: ["staff-client-portal-inbox", organizationId] }); client.invalidateQueries({ queryKey: ["staff-client-portal-service-center", organizationId] }); client.invalidateQueries({ queryKey: ["team-members", organizationId] }); if (threadId) client.invalidateQueries({ queryKey: ["communication-entries", threadId] }); }; }
...
export function useClaimPortalCommunicationThread(organizationId: string | null) { const refresh = useRefresh(organizationId); return useMutation({ mutationFn: async (threadId: string) => { const { error } = await db().rpc("claim_portal_communication_thread", { _thread_id: threadId }); if (error) throw error; }, onSuccess: (_, threadId) => refresh(threadId) }); }
export function useUpdateCommunicationThread(organizationId: string | null) { const refresh = useRefresh(organizationId); return useMutation({ mutationFn: async ({ threadId, priority, processId, taskId, followUpAt, clearFollowUp = false }: { threadId: string; priority?: CommunicationPriority; processId?: string | null; taskId?: string | null; followUpAt?: string | null; clearFollowUp?: boolean }) => { const { error } = await db().rpc("update_communication_thread", { _thread_id: threadId, _priority: priority ?? null, _process_id: processId ?? null, _process_id_provided: processId !== undefined, _task_id: taskId ?? null, _task_id_provided: taskId !== undefined, _follow_up_at: followUpAt || null, _clear_follow_up: clearFollowUp }); if (error) throw error; }, onSuccess: (_, v) => refresh(v.threadId) }); }
