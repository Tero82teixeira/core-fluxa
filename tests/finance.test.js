...
import { brDate, brl, financialBuckets, financialCsv, financeViewState } from "../src/lib/finance.ts";
...
describe("módulo financeiro",()=>{
 ...
 test("primeiro render com data indefinida permanece no loading",()=>assert.equal(financeViewState("org-1",{data:undefined,isLoading:true,isError:false}),"loading"));
 test("render de erro com data indefinida permanece seguro",()=>assert.equal(financeViewState("org-1",{data:undefined,isLoading:false,isError:true}),"error"));
 test("dados derivados são montados apenas no componente carregado",()=>{assert.match(route,/if\(!query\.data\)return null;\s*return <FinanceContent/);assert.match(route,/function FinanceContent/)});
