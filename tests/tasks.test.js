...
const migration = readFileSync(
  new URL("../supabase/migrations/20260802190000_complete_tasks_module.sql", import.meta.url),
  "utf8",
);
const tasksRoute = readFileSync(
  new URL("../src/routes/_authenticated/tarefas.tsx", import.meta.url),
  "utf8",
);
...
describe("módulo de tarefas", () => {
  ...
  test("identifica archived_at", () =>
    assert.equal(isTaskArchived(task({ archived_at: "2026-08-01" })), true));
  test("identifica status arquivada", () =>
    assert.equal(isTaskArchived(task({ status: "arquivada" })), true));
  test("não arquivada permanece ativa", () => assert.equal(isTaskArchived(task()), false));
  test("identifica atraso", () =>
  ...
  test("extrai dia UTC da agenda", () =>
    assert.equal(taskDateKey("2026-08-02T22:10:00Z"), "2026-08-02"));
  ...
  test("filtra abertas", () =>
    assert.equal(
      filterTasks([task(), task({ status: "concluida" })], { status: "open" }).length,
      1,
    ));
  test("filtra prioridade", () =>
    assert.equal(
      filterTasks([task(), task({ priority: "alta" })], { priority: "alta" })[0].priority,
      "alta",
    ));
  test("filtra responsável", () =>
    assert.equal(
  ...
  test("atalho de arquivadas remove o conflito com Em aberto", () =>
    assert.deepEqual(nextTaskArchiveView(false), { archived: true, status: "all" }));
  test("saída de arquivadas restaura Em aberto", () =>
    assert.deepEqual(nextTaskArchiveView(true), { archived: false, status: "open" }));
  test("atalho exibe tarefa com status arquivada", () => {
    const filters = nextTaskArchiveView(false);
    assert.equal(
      filterTasks([task({ status: "arquivada", archived_at: "2026-08-01" })], filters).length,
      1,
    );
  });
  ...
  test("calcula indicadores", () =>
    assert.deepEqual(
      taskIndicators(
        [task(), task({ status: "concluida" }), task({ status: "arquivada" })],
        new Date("2026-08-02"),
      ),
      { open: 1, overdue: 1, completed: 1, archived: 1 },
    ));
  test("agrupa colunas do quadro", () =>
    assert.equal(
      groupTasksByStatus([task(), task({ status: "aguardando" })]).aguardando.length,
      1,
    ));
  test("inclui em andamento no payload de atualização", () =>
    assert.equal(buildTaskStatusUpdate("em_andamento", "user-1").status, "em_andamento"));
  test("inclui aguardando no payload de atualização", () =>
    assert.equal(buildTaskStatusUpdate("aguardando", "user-1").status, "aguardando"));
  test("conclusão registra autor e instante", () =>
    assert.deepEqual(buildTaskStatusUpdate("concluida", "user-1", "2026-08-16T12:00:00.000Z"), {
      status: "concluida",
      completed_at: "2026-08-16T12:00:00.000Z",
      completed_by: "user-1",
    }));
  test("reabertura limpa os campos de conclusão", () =>
    assert.deepEqual(buildTaskStatusUpdate("pendente", "user-1"), {
      status: "pendente",
      completed_at: null,
      completed_by: null,
    }));
  test("edição legada sem status preserva o status atual", () =>
    assert.deepEqual(buildTaskStatusUpdate(undefined, "user-1"), {}));
});

describe("segurança SQL de tarefas", () => {
  test("bloqueia responsável de outra organização", () =>
    assert.match(
      migration,
      /organization_members[\s\S]+m\.organization_id = NEW\.organization_id[\s\S]+TASK_ASSIGNEE_NOT_MEMBER/,
    ));
  test("bloqueia cliente de outra organização", () =>
    assert.match(
      migration,
      /clients c[\s\S]+c\.organization_id = NEW\.organization_id[\s\S]+TASK_CLIENT_ORG_MISMATCH/,
    ));
  test("bloqueia processo de outra organização", () =>
    assert.match(
      migration,
      /processes p[\s\S]+p\.organization_id = NEW\.organization_id[\s\S]+TASK_PROCESS_ORG_MISMATCH/,
    ));
  test("bloqueia documento de outra organização", () =>
    assert.match(
      migration,
      /documents d[\s\S]+d\.organization_id = NEW\.organization_id[\s\S]+TASK_DOCUMENT_ORG_MISMATCH/,
    ));
  test("bloqueia monitoramento de outra organização", () =>
    assert.match(
      migration,
      /monitoring_items m[\s\S]+m\.organization_id = NEW\.organization_id[\s\S]+TASK_MONITORING_ORG_MISMATCH/,
    ));
  test("organization_id da tarefa é imutável", () =>
    assert.match(
      migration,
      /OLD\.organization_id <> NEW\.organization_id[\s\S]+TASK_ORGANIZATION_IMMUTABLE/,
    ));
  test("comentário não pode ser movido de organização", () =>
    assert.match(migration, /task_comments_enforce_scope_trg BEFORE INSERT OR UPDATE/));
  test("exclusão física está bloqueada", () => {
    assert.match(migration, /REVOKE DELETE ON public\.tasks FROM authenticated/);
    assert.doesNotMatch(migration, /CREATE POLICY tasks_delete/);
  });
  test("arquivamento lógico permanece disponível", () =>
    assert.match(
      migration,
      /ADD COLUMN IF NOT EXISTS archived_at[\s\S]+ADD COLUMN IF NOT EXISTS deleted_at/,
    ));
});
